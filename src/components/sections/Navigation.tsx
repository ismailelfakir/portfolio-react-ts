  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
        isScrolled ? 'bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-700 shadow-lg' : 'bg-transparent'
      }`}
    >
        ...
        <div className="flex items-center justify-between h-16">
          ...
          >
            ...
            <div className="flex flex-col">
              <span className="text-lg font-bold gradient-text-primary leading-tight">
                {personalInfo.name}
              </span>
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              {isMobileMenuOpen ? (
                <X className="absolute h-4 w-4 text-slate-700 dark:text-slate-200" />
              ) : (
                <Menu className="absolute h-4 w-4 text-slate-700 dark:text-slate-200" />
              )}
            </Button>
          </div>
        </div>
